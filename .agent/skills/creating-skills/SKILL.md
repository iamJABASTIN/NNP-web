...
- When the user wants to "build a skill" or "generate a skill" or "create a skill".

## Workflow
1.  **Analyze Request**: Understand the user's goal for the new skill.
2.  **Verify Name**: Ensure the skill name is in gerund form (e.g., `testing-code`).
3.  **Create Directory**: Make the directory `.agent/skills/<skill-name>/`.
4.  **Draft Content**: Create the content for `SKILL.md` following the specific guidelines below.
5.  **Write File**: Write the `SKILL.md` file.
6.  **Create Extras**: Add `scripts/` or `examples/` if necessary.
