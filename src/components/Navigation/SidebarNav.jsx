...
const SidebarNav = ({ activeTab, onTabChange, hasActiveOrder }) => {
  ...
  const tabs = [
    ...
    { id: 'orders', icon: ClipboardList, label: 'My Orders', badge: hasActiveOrder },
    ...
    <div className="hidden md:block fixed left-0 top-0 h-full z-[80]">
      ...
      >
        ...
        <button 
          ...
          className="mb-12 p-4 border-2 border-black bg-accent text-black hover:scale-105 active:scale-95 transition-all shadow-[4px_4px_0px_#000000] flex items-center justify-center rounded-xl mx-auto"
        >
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-4 p-4 rounded-xl border-2 transition-all duration-300 group ${
                  isActive 
              ...
              >
                <div className="relative">
                  ...
                  {tab.badge && !isOpen && (
                    <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 border-2 border-white rounded-full ring-2 ring-black" />
                  )}
                </div>
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      ...
                      {tab.badge && (
                        <span className="px-2 py-0.5 bg-red-500 text-white text-[8px] font-black uppercase rounded-full">Active</span>
                      )}
                ...
                {isActive && !isOpen && (
                  <motion.div 
                    ...
                    className="absolute -right-6 w-2 h-8 bg-accent border-l-2 border-black" 
                  />
                )}
              </button>
            );
      ...
      {/* Overlay for mobile/sidebar when open (optional if using fully desktop side) */}
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            initial={{ opacity: 0 }}
