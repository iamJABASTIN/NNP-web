  ...
  return (
    ...
    >
      <div className="aspect-square bg-muted/20 border-b-4 border-black relative overflow-hidden flex-shrink-0">
         {item.image_url ? <img src={item.image_url} alt={tField(item, 'name')} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /> : <UtensilsCrossed className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-black/10" size={32} />}
         ...
         <div className="absolute top-2 right-2 z-10">
            <div className={`w-5 h-5 border-2 border-black flex items-center justify-center bg-white shadow-[2px_2px_0px_#000000] ${item.veg_type === 'veg' ? 'bg-green-100' : 'bg-red-100'}`}>
              <div className={`w-2 h-2 ${item.veg_type === 'veg' ? 'bg-green-600' : 'bg-red-600'}`} />
            </div>
         </div>
      </div>
